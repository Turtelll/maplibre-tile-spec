...
import { decodeColumnTypeFromCode, doesColumnTypeHaveExplicitName, doesColumnTypeHaveChildren } from "./typeMap";
...
function decodeColumn(src: Uint8Array, offset: IntWrapper): Column {
    ...
    const column = decodeColumnTypeFromCode(typeCode);

    if (!column) {
        throw new Error(`Unsupported column type code: ${typeCode}`);
    }

    if (doesColumnTypeHaveExplicitName(typeCode)) {
    ...
    if (doesColumnTypeHaveChildren(typeCode)) {
